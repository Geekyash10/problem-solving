...
{
    int mostLastToBeUsedPage = -1;
    int mostLatestIndex = -1;
    for (auto page : memory)
    {
       ...
       //jo index peeche hain mtlb vo least recent hoga toh vo index and page note krlo 
        if (index > mostLatestIndex)
        {
            mostLatestIndex = index;
            mostLastToBeUsedPage = page;
  ...
  for(int i = 0 ; i < memory.size(); i++){
    if(memory[i] == mostLastToBeUsedPage){
        return i;
    }
  }
  return -1;
...
void optimalPageReplacement(int frames, vector<int> pages)
{
    vector<int> memory;
    ...
    for (int i = 0; i < pages.size(); i++)
    {
        ...
        else
        {
            ...
            // Print the current state of memory    cout << "Memory: ";
            for (int page : memory)
            {
                cout << page << " ";
            }
...
int main()
{
    ...
    optimalPageReplacement(frames, pages);
